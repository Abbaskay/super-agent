  ...
  <div class="auth-page">
    <div class="auth-container">
      ...
      <div class="auth-header">
        ...
        <p class="auth-subtitle">Get started with Zonrad</p>
      </div>

      <form class="auth-form" @submit.prevent="handleSignup">
        <div class="form-group">
          <label class="form-label">Full name</label>
          <input v-model="name" type="text" class="form-input" placeholder="John Doe" required autocomplete="name">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <input v-model="password" type="password" class="form-input" placeholder="Create a password (min 8 characters)" required autocomplete="new-password" minlength="8">
        </div>
...
export default {
  ...
  methods: {
    socialAuth,
    async handleSignup() {
      console.log('[SignupView] handleSignup', { name: this.name, email: this.email })
      ...
      try {
        ...
        console.log('[SignupView] registration successful')
        authState.login(res.data.token, res.data.user)
        console.log('[SignupView] redirecting to /')
        this.$router.push('/')
      } catch (e) {
        ...
        console.error('[SignupView] registration failed', { error: msg })
        this.error = msg
        this.loading = false
      }
    },
